...
tr.on('exit', function (code) {
    ...
    fs.readFile(file, function (err, src) {
        ...
        var setTimeout_ = function (f, ts) {
            pending ++;
            var to = setTimeout(function () {
                f.apply(this, arguments);
                checkNext();
            }, ts);
            tr.once('exit', function () {
                clearTimeout(to);
            });
            return to;
        };
        
        var setInterval_ = function (f, ts) {
            pending ++;
            var iv = setInterval(f, ts);
            tr.once('exit', function () {
console.log('caught exit', iv);
                clearInterval(iv);
            });
            return iv;
        };
        var clearInterval_ = function (iv) {
            checkNext();
            return clearInterval(iv);
        };
        var checkNext = function () {
            nextTick(function () {
                if (--pending === 0) pr.exit();
            });
        };
        
        var pending = 0;
        Function(
            [ 'console', 'require', 'process', 'setInterval', 'setTimeout' ],
            src
        )(con, require_, pr, setInterval_, setTimeout_, clearInterval_);
...
exports.vim = exports.vi = function (fs, args, opts) {
    ...
    vim.view.on('change', function () {
        ...
        for (var i = 0; i < vis[1]; i++) {
            if (lines[i] && rows[i] === lines[i]) continue;
...
exports.date = require('./commands/date.js');
//exports.git = require('./commands/git.js');
